...
import com.bloobirds.dashboards.datamodel.service.ContactService;
...
public class ContactView extends Div {
    ...
    private static final SerializableBiConsumer<Badge, Contact> statusComponentUpdater = (badge, contact) -> {
        Integer status = contact.getStatus();

        switch (status) {
            case Contact.STATUS_ON_PROSPECTION -> { badge.setText(User.getMessage("contact.status.onprospection")); badge.setVariant(Badge.BadgeVariant.CONTRAST); }
            case Contact.STATUS_CONTACTED -> { badge.setText(User.getMessage("contact.status.contacted")); badge.setVariant(Badge.BadgeVariant.SUCCESS); }
            case Contact.STATUS_ENGAGED -> { badge.setText(User.getMessage("contact.status.engaged")); badge.setVariant(Badge.BadgeVariant.SUCCESS); }
            case Contact.STATUS_MEETING -> { badge.setText(User.getMessage("contact.status.meeting")); badge.setVariant(Badge.BadgeVariant.SUCCESS); }
            case Contact.STATUS_ACCOUNT -> { badge.setText(User.getMessage("contact.status.account")); badge.setVariant(Badge.BadgeVariant.NORMAL); }
            case Contact.STATUS_NURTURING -> { badge.setText(User.getMessage("contact.status.nurturing")); badge.setVariant(Badge.BadgeVariant.CONTRAST); }
            case Contact.STATUS_DISCARDED -> { badge.setText(User.getMessage("contact.status.discarded")); badge.setVariant(Badge.BadgeVariant.ERROR); }
            default -> {
                String label = User.getMessage("contact.status." + status);
                if (label == null) label = User.getMessage("salesuser.status.nostatus");
                badge.setText(label); badge.setVariant(Badge.BadgeVariant.CONTRAST); }

        }
    };

    public ContactView(@Autowired ContactService service) {

        var grid = new EnhancedGrid<Contact>();
        grid.setSelectionMode(Grid.SelectionMode.SINGLE);
        grid.addColumn(contact -> contact.getObjectID().getTenantID()).setHeader(User.getMessage("tenant")).setAutoWidth(true).setResizable(true);
        ...
        grid.addColumn(Contact::getName).setHeader(User.getMessage("contact.name")).setAutoWidth(true);
        grid.addColumn(Contact::getSurname).setHeader(User.getMessage("contact.surname")).setAutoWidth(true);
        grid.addColumn(Contact::getJobTitle).setHeader(User.getMessage("contact.role")).setAutoWidth(true);
        grid.addColumn(Contact::getEmail).setHeader(User.getMessage("contact.email")).setAutoWidth(true);
        grid.addColumn(createStatusComponentRenderer()).setHeader(User.getMessage("contact.status")).setAutoWidth(true);
        grid.addColumn(Contact::getAssignToFullName).setHeader(User.getMessage("contact.assignto")).setAutoWidth(true);

        int fields= Integer.parseInt(User.getMessage("contact.datamodel.fields"));
        if(fields>0) {
            ...
            grid.addComponentColumn(contact -> {
                ...
                details.addClickListener(e -> {
                    ...
                    dialog.getElement().setAttribute("aria-label", User.getMessage("contact.details"));
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            Contact current = event.getValue();
// @TODO wrong!!
            if (current == null) current = new Contact();

            Contact old = event.getOldValue();
            if (old == null) old = new Contact();
    ...
    private static ComponentRenderer<Badge, Contact> createStatusComponentRenderer() {
        return new ComponentRenderer<>(Badge::new, statusComponentUpdater);
    }


    private VerticalLayout createDialogLayout(Contact person, Dialog dialog) {
        H2 headline = new H2(User.getMessage("contact.details"));
        ...
        for (int i=0;i<fields;i++){
            String field= User.getMessage("contact.field."+i);
    ...
    @Autowired
    private SalesUserService userService;

    private void generateTestData(ContactService service) {

        List<SalesUser> users = userService.findAll();

        BBObjectID BBid= new BBObjectID();
        Random rand= new Random();
        BBid.setTenantID("bloobirds");
        Contact c = new Contact();
        c.setLinkedIn("https://www.linkedin.com/company/bloobirds/");
        ...
        c.setEmail(c.getName().substring(0)+c.getSurname()+"@spam.info");
        c.setJobTitle(roles[rand.nextInt(10)]+" "+functions[rand.nextInt(6)]);
        c.setStatus(rand.nextInt(9));
        c.setAssignTo(users.get(rand.nextInt(users.size())));
        c.addAttribute(User.getMessage("contact.field.0"),rates[rand.nextInt(7)]);
        c.addAttribute(User.getMessage("contact.field.1"),sources[rand.nextInt(5)]);
        c=service.save(c);
    ...
    String[] sources = {"Outbound","Inbound","Sales","Referral","Partner"};
}
